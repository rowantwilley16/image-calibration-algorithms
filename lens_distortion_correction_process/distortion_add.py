...
TODO: when using this file make sure to make folders for the M_vectors and figures to save the files in!

"""
# =========================== SETUP BEFORE RUNNING ============================

#save folders and files
m_vectors_save_folder           = "4000x4000"
input_image_file                = "4000x4000_sat_img_1.jpg"

# Find the position of the underscore and the dot
underscore_position = input_image_file.find('_')
dot_position = input_image_file.find('.')

# Slice the string between the underscore and the dot
file_name_only = input_image_file[underscore_position + 1:dot_position]

figures_save_folder             = m_vectors_save_folder + "_" + file_name_only
distorted_images_save_folder    = m_vectors_save_folder + "_" + file_name_only

#image dimensions
image_width     = 4000
image_height    = 4000

filter_offset_from_centre      = 1000
...
input_image_path = fr"C:\Users\rowan\OneDrive\Desktop\code\image-calibration-algorithms\lens_distortion_correction_process\input_images\{input_image_file}"
...
TDI_stages = 4
filter_step_size = 2 # loop incrementor
...
if plot_figures: 
    ...
    if(yesno=='Y'):
        ...
        plt.savefig(fr"C:\Users\rowan\OneDrive\Desktop\code\image-calibration-algorithms\lens_distortion_correction_process\figures\{figures_save_folder}\M_vector_x_comp_varying_spectral_filter_locations.pdf")
    ...
    if(yesno=='Y'):
        ...
        plt.savefig(fr"C:\Users\rowan\OneDrive\Desktop\code\image-calibration-algorithms\lens_distortion_correction_process\figures\{figures_save_folder}\M_vector_y_comp_varying_spectral_filter_locations.pdf")
    ...
    if(yesno=='Y'):
        ...
        plt.savefig(fr"C:\Users\rowan\OneDrive\Desktop\code\image-calibration-algorithms\lens_distortion_correction_process\figures\{figures_save_folder}\M_vector_legend.pdf")
    ...
    if(yesno=='Y'):
        ...
        plt.savefig(fr"C:\Users\rowan\OneDrive\Desktop\code\image-calibration-algorithms\lens_distortion_correction_process\figures\{figures_save_folder}\radial_terms.pdf")
...
if draw_output_image: 
    ...
    middle_of_sensor        = cy 
    #filter offset from centre set at top of the script
    blue_filter_location    = middle_of_sensor - filter_offset_from_centre
    green_filter_location   = middle_of_sensor 
    red_filter_location     = middle_of_sensor + filter_offset_from_centre
    ...
    M_vector_blue_list = [M_vector_x_window_blue, M_vector_y_window_blue]

    #save M_vector blue to a text file
    print("saving the M_vector blue to a text files ...")
# Save M_vector_x_window_blue
    np.savetxt(
        fr"C:\Users\rowan\OneDrive\Desktop\code\image-calibration-algorithms\lens_distortion_correction_process\M_vectors\{m_vectors_save_folder}\Mx_blue.txt",
        M_vector_x_window_blue,
        fmt='%lf')


# Save M_vector_y_window_blue
    np.savetxt(
        fr"C:\Users\rowan\OneDrive\Desktop\code\image-calibration-algorithms\lens_distortion_correction_process\M_vectors\{m_vectors_save_folder}\My_blue.txt",
        M_vector_y_window_blue,
        fmt='%lf')
    ...
    for y in range(height):
        for x in range(width):
            ...
            new_x = current_pixel_x + pixel_x_movement_blue[x]  #x is the column index
            new_y = current_pixel_y + pixel_y_movement_blue[x] + padding_y #x is the column index

            #set the pixel value in the new image
            blue_image_distorted[math.floor(new_y), math.floor(new_x)] = pixel_value #the math. ceil causes black lines in the iamge(interpolation issue)
    ...
    M_vector_green_list = [M_vector_x_window_green, M_vector_y_window_green]
    
    #save M_vector green to a text file
    print("saving the M_vector green to a text file ...")
    np.savetxt(fr"C:\Users\rowan\OneDrive\Desktop\code\image-calibration-algorithms\lens_distortion_correction_process\M_vectors\{m_vectors_save_folder}\Mx_green.txt", 
               M_vector_x_window_green, 
               fmt='%lf')
    np.savetxt(fr"C:\Users\rowan\OneDrive\Desktop\code\image-calibration-algorithms\lens_distortion_correction_process\M_vectors\{m_vectors_save_folder}\My_green.txt",
               M_vector_y_window_green, 
               fmt='%lf')
    #floating point movement values for the pixels(original to distorted image)
    ...
    for y in range(height):
        for x in range(width):
            ...
            #get the new x and y coordinates -this needs to change!Wont be max distortion but based on the M vector instead.
            new_x = current_pixel_x + pixel_x_movement_green[x]  #x is the column index
            new_y = current_pixel_y + pixel_y_movement_green[x] + padding_y #x is the column index

            #set the pixel value in the new image
            green_image_distorted[math.floor(new_y), math.floor(new_x)] = pixel_value #the math. ceil causes black lines in the iamge(interpolation issue)
    ...
    M_vector_red_list = [M_vector_x_window_red, M_vector_y_window_red]

    #save
    print("saving the M_vector red to a text file ...")
    np.savetxt(fr"C:\Users\rowan\OneDrive\Desktop\code\image-calibration-algorithms\lens_distortion_correction_process\M_vectors\{m_vectors_save_folder}\Mx_red.txt", 
               M_vector_x_window_red, 
               fmt='%lf')
    np.savetxt(fr"C:\Users\rowan\OneDrive\Desktop\code\image-calibration-algorithms\lens_distortion_correction_process\M_vectors\{m_vectors_save_folder}\My_red.txt", 
               M_vector_y_window_red, 
               fmt='%lf')
    ...
    for y in range(height):
        for x in range(width):
            ...
            #get the new x and y coordinates -this needs to change!Wont be max distortion but based on the M vector instead.
            new_x = current_pixel_x + pixel_x_movement_red[x]  #x is the column index
    ...
    np.savetxt(fr"C:\Users\rowan\OneDrive\Desktop\code\image-calibration-algorithms\lens_distortion_correction_process\distorted_images\{distorted_images_save_folder}\blue_image_distorted.txt", 
               blue_image_distorted, 
               fmt='%d')
    ...
    np.savetxt(fr"C:\Users\rowan\OneDrive\Desktop\code\image-calibration-algorithms\lens_distortion_correction_process\distorted_images\{distorted_images_save_folder}\green_image_distorted.txt", 
               green_image_distorted, 
               fmt='%d')   
    ...
    np.savetxt(fr"C:\Users\rowan\OneDrive\Desktop\code\image-calibration-algorithms\lens_distortion_correction_process\distorted_images\{distorted_images_save_folder}\red_image_distorted.txt", 
               red_image_distorted, 
               fmt='%d')
    ...
    if(yesno=='Y'):
        ...
        plt.savefig(fr"C:\Users\rowan\OneDrive\Desktop\code\image-calibration-algorithms\lens_distortion_correction_process\figures\{figures_save_folder}\color_channels_distorted.pdf")
    ...
    if(yesno=='Y'):
        ...
        plt.savefig(fr"C:\Users\rowan\OneDrive\Desktop\code\image-calibration-algorithms\lens_distortion_correction_process\figures\{figures_save_folder}\simulated_distorted_image.pdf")
